...
def upload(request):
    ...
    if request.method == 'POST':
        uploader = BookCreate(request.POST, request.FILES)
        if uploader.is_valid():
            uploader.save()
            return redirect('index')
        else:
            return HttpResponse(
                """ Something went wrong. Please reload the webpage by clicking <a href="{{url:'index'}}>Reload</a>" """)
    else:
        return render(request, 'book/upload_form.html', {'upload_form': uploader})
